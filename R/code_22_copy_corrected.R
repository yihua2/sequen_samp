g_part <- cbind(1,c(1,rep(0,9)),c(-1,rep(0,9),c(0,1,1,rep(0,7)),c(0,-1,rep(0,8)),c(0,0,-1,rep(0,7))))
...
#contrast matrix
g_part = cbind(rep(1,10),
               rbind(c(-1,1,rep(0,12)),
...
theta =  ginv(g_part)  %*% theta1
hist(pnorm(X%*%theta),xlim = c(0,1),breaks = 11)
mean(pnorm(X%*%theta)) #
which.max(pnorm(X%*%theta)) #99-0.7956535
...
for (R in 1:N){
  ...
  d1d <- opt_design(design_stg1,theta_stg1 ,n=600,pp=0,space)
 ...
 # g_part <- diag(rep(1,15))
  ...
  d1d2 <- opt_design(design_stg2,theta_hat_pos ,n=2400,pp=0,space)
...
#2 17 21 22 37 82 87 
#13  4  1 72  8  1  1 
save.image("/Users/yihua/Documents/GitHub/sequen_samp/R/code_22_copy_corrected_img.RData")
